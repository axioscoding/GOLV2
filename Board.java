...
public class Board {
    ...
    public void render2(Graphics2D g) {
                    ...
                    if (GameOfLife.COLORS) {
                        if (GameOfLife.RAND_COLORS) {
                            g.setColor(new Color(Color.HSBtoRGB(
                                    (((float) y / (float) height) / 2 + ((float) x / (float) width) / 2)
                                            * (float) (Math.random() * 0.8f + 0.2),
                                    1.0f, GameOfLife.INVERTED_COLORS ? 0.8f : 1.0f)));
                        } else {
                            g.setColor(new Color(
                                    Color.HSBtoRGB((((float) y / (float) height) / 2 + ((float) x / (float) width) / 2),
                                            1.0f, (GameOfLife.INVERTED_COLORS ? 0.8f : 1.0f))));
                        }
                    } else {
                        if (GameOfLife.INVERTED_COLORS)
        ...
        // g.setColor(Color.WHITE);
        // g.fillRect(tupel.getX() * GameOfLife.CELL_SIZE, tupel.getY() *
        // GameOfLife.CELL_SIZE, GameOfLife.CELL_SIZE, GameOfLife.CELL_SIZE);
        // }
    ...
    static class Presets {
        ...
        static final void heptomino_swarm(Board b) {
            for (int x = 0; x < 20; x++) {
                for (int y = 0; y < 10; y++) {
                    b.loadPattern(PatternBank.E_HEPTOMINO, (int) (b.width * ((double) x / 20)),
                            (int) (b.height * ((double) y / 10)));
                }
            }
        }
