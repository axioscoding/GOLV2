...
public class Converter {
    ...
    public static final Pattern fromRLE(String input) {
        ...
        if (!(parts[iterator].contains("x") && parts[iterator].contains("y")))
            throw new IllegalArgumentException("Wrong Format 2!");

        // if(!parts[iterator].contains("B3/S23")) throw new
        // IllegalArgumentException("Wrong Rule!");
        ...
        if ((dimX >= GameOfLife.WIDTH / GameOfLife.CELL_SIZE - 20)
                || dimY >= GameOfLife.HEIGHT / GameOfLife.CELL_SIZE - 20)
    ...
    public static String[] decode(final String st, int w) {
        ...
        while (i < chars.length) {
            ...
            if (i == chars.length && repeat > 0 && s.toString().isEmpty()) {
